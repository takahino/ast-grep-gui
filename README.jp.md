...
- 対応言語では検索結果に型ヒント列を表示（構文ベース・best-effort）：単一メタ変数（`$NAME`）は列ごとに、複数ノードキャプチャ（`$$$ARGS` など）は **`NAME#arity`**（キャプチャ個数・例: 呼び出しの引数数）と **`NAME#0`**、**`NAME#1`** …（各ノードの型）の列。**C++** はディスク上の `#include` を辿ってヘッダ内の宣言を参照できます。**詳細設定**（AST 系モード時）で **`;` 区切りのインクルードディレクトリ**（コンパイラの `-I` 相当）を追加すると、標準・SDK ヘッダなども型ヒント解決に利用しやすくなります。**型ヒント補助設定**（YML）で、メソッド戻り型・マクロ・定数・フィールド・二項演算子などを明示的に登録できます。
...
3. 検索モードを選びます。
4. `AST` モードでは対象言語を選ぶか `Auto` を使います。
5. AST パターン、トークン列、文字列、または正規表現を入力します。
6. 必要に応じてコンテキスト行数、ファイルフィルタ、文字コード、スキップディレクトリ、各モード固有オプション、（AST 系モードでは）**詳細設定**（**C++ のインクルードパス** など）を調整します。
...
AST 系モードで型ヒントが有効なとき、**詳細設定**の **型補助設定...** から、C++ 向けの補助ルールを GUI で編集できます。YML の **読み込み** / **保存** も同じ画面から行えます。ルールはアプリ再起動後も保持されます（手動で YML を保存した場合はそのファイルパスも記憶）。編集画面の **`params` は 1 行に 1 型**（例: `LPCTSTR` を単独行で入力）で記入します。YML ファイル側は従来どおり `params` 配列です。
...
型ヒント列が付くパターンでは、`JSON`、`Markdown`、`HTML`、`Excel` の出力にも表ビューと同様の列（`NAME#arity` や `NAME#i` を含む）が含まれます。
...
src/receiver_hint.rs     メタ変数向け best-effort 型ヒント（言語別。C++ は追加インクルードパス・補助 YML を利用可）
src/type_hint_config.rs  型ヒント補助ルールの YML スキーマ・照合・ドラフト生成
...
直近の開発でユーザー向けに効く変更です（すべては `git log` で確認できます）。

- **リモート Git/SVN 検索:** ローカル clone なしでリモート URL を検索対象にできる。ref/revision・サブディレクトリ指定、ローカルキャッシュと再取得に対応。
- **関連付けで開く:** ファイル一覧・コードヘッダー・表のアクション列からヒットファイルを既定アプリで開ける。
- **C++ 型ヒント:** 詳細設定で **`;` 区切りのインクルードディレクトリ**（`-I` 相当）を指定可能に。`#include` 解決で標準・SDK ヘッダへ届きやすくなりました。
- **型ヒント補助設定:** C++ 向けにメソッド／マクロ／定数／フィールド／二項演算子の型を YML で登録。GUI 編集、YML 入出力、**表ビュー**および**サマリー**の型ヒントセル（未解決または `Class.Method` 表示）から右クリックで設定画面へ遷移（自動入力付き。methods では arity／params も同一行から反映）。GUI の `params` は改行区切り（1 行 1 型）。
- **サマリー表示:** 推定された受信側の型・引数数・各引数の型などを集計（パターン次第でメソッド列も）。
- **表モード:** 型ヒント列のリサイズ、固定ヘッダー、キーボード横スクロール、空セルと不明の区別などを改善。
