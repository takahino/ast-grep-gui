...
fn rust_probes(lang: UiLanguage) -> Vec<(String, String)> {
    vec![
        probe_l(
            ...
            "fn $NAME($$$ARGS)",
            "関数シグネチャ（名前と引数）",
            "Function signature (name and args)",
        ),
        probe_l(
            lang,
            "fn $NAME($$$ARGS) { $$$BODY }",
...
fn java_probes(lang: UiLanguage) -> Vec<(String, String)> {
    vec![
        ...
        probe_l(
            lang,
            "try { $$$BODY } catch ($EX) { $$$CATCH }",
            "try-catch",
            "try-catch",
        ),
        probe_l(
            lang,
            "try { $$$BODY } finally { $$$FINALLY }",
            "try-finally",
            "try-finally",
        ),
        probe_l(lang, "return $EXPR;", "return文", "return"),
...
fn js_probes(lang: UiLanguage) -> Vec<(String, String)> {
    vec![
        ...
        probe_l(
            lang,
            "for (let $VAR = $INIT; $COND; $UPDATE) { $$$BODY }",
            "for ループ",
            "for loop",
        ),
        probe_l(
...
fn go_probes(lang: UiLanguage) -> Vec<(String, String)> {
    vec![
        ...
        probe_l(
            lang,
            "for $KEY, $VAL := range $ITER { $$$BODY }",
            "range ループ",
            "range (key,val)",
        ),
        probe_l(
            lang,
            "for $VAL := range $ITER { $$$BODY }",
            "range ループ（単値）",
            "range (val)",
        ),
        probe_l(lang, "switch $EXPR { $$$BODY }", "switch 文", "switch"),
...
fn cpp_probes(lang: UiLanguage) -> Vec<(String, String)> {
    ...
    probes.extend(vec![
        ...
        probe_l(
            ...
            "$CLASS::$METHOD()",
            "スコープ解決付き呼び出し（0引数）",
            "scope-qualified call (0 args)",
        ),
        probe_l(
            lang,
            "$CLASS::$METHOD($ARG)",
            "スコープ解決付き呼び出し（1引数）",
            "scope-qualified call (1 arg)",
        ),
        probe_l(
            lang,
...
mod probe_pattern_syntax {
    ...
    fn all_fixed_languages_probe_patterns_compile() {
        ...
        for lang in langs {
            let ast_lang = lang
                .to_support_lang()
                .expect("fixed language maps to SupportLang");
