...
fn file_filter_display<'a>(t: Tr, cond: &'a SearchConditions) -> std::borrow::Cow<'a, str> {
...
/// Markdown 向け（見出し `##` 付き）
fn format_search_conditions_markdown(t: Tr, cond: &SearchConditions, lang: UiLanguage) -> String {
    let mut s = String::new();
    s.push_str("## ");
    s.push_str(t.export_conditions_title());
    s.push_str("\n\n");
