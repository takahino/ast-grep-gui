...
use crate::search::{compute_cpp_include_path_diagnostics, cpp_include_diagnostic_cache_key};
    ...
    egui::CollapsingHeader::new(t.cpp_include_diagnostic_header())
        ...
        .show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ui.label(
                    egui::RichText::new(
                        t.cpp_include_diagnostic_cpp_files(diag.distinct_cpp_result_files),
                    )
                    .small(),
                );
            });
            ...
            if diag.source_read_errors > 0 {
                ui.label(
                    egui::RichText::new(
                        t.cpp_include_diagnostic_read_errors(diag.source_read_errors),
                    )
                    .small()
